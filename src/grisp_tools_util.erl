...
copy_directory(State0, Src, DestRoot, DestRel, Opts) ->
    recursive_copy(State0, Src, DestRoot, DestRel, [], Opts).
...
%% quotes with the standard POSIX shell pattern.
...
recursive_copy(State0, Src, DestRoot, DestRel, RevRelNames, Opts) ->
    {SrcPath, RelPath} = case RevRelNames of
        [] -> {Src, DestRel};
        _ ->
            R = filename:join(lists:reverse(RevRelNames)),
            {filename:join(Src, R), filename:join(DestRel, R)}
    end,
    case file:read_file_info(SrcPath, [{time, posix}]) of
        {ok, #file_info{type = directory} = FileInfo} ->
            State1 = recursive_create_dir(State0, DestRoot, RelPath, FileInfo, Opts),
            case file:list_dir(SrcPath) of
                ...
                {ok, Names} ->
                    lists:foldl(fun(Name, State) ->
                        ...
                        recursive_copy(State, Src, DestRoot, DestRel,
                                       RevRelNames2, Opts)
                    end, State1, Names)
            end;
        {ok, #file_info{type = regular} = FileInfo} ->
            case file:read_link_info(SrcPath, [{time, posix}]) of
                {ok, #file_info{type = regular}} ->
                    recursive_copy_file(State0, SrcPath, DestRoot, RelPath,
                                        FileInfo, Opts);
                {ok, #file_info{type = symlink}} ->
                    SrcPath2 = resolve_symlink(SrcPath),
                    recursive_copy_file(State0, SrcPath2, DestRoot, RelPath,
                                        FileInfo, Opts);
